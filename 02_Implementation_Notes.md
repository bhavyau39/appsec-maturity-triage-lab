...
- Since Nessus Essentials cannot export CSV, manually curated these findings into `nessus_findings.xlsx` with:
...
- Created `AppSec_Maturity_Model.xlsx` with a simple 1–5 scoring model inspired by OWASP ASVS.
...
- Created one work item per curated finding from `nessus_findings.xlsx`:
